class ClientWorkoutsController < ApplicationController
  ...
  def find
    #@client_workouts = ClientWorkout.where("client_name = ? OR trainer = ?", params[:search_string], params[:search_string]) #どちらか
    #@client_workouts = ClientWorkout.where("client_name like ? ", "%" + params[:search_string] + "%") #あいまい検索
    @client_workouts = ClientWorkout.where("client_name like ? OR trainer like ?", "%" + params[:search_string] + "%", "%" + params[:search_string] + "%")
  end
